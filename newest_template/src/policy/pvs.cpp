...
int move_order_score(State *state, const Move& move){
    ...
    if(attacker == 1 && (tr == 0 || tr == state->board_h() - 1)){
        score += 800;
    }
...
std::vector<Move> ordered_actions(State *state){
...
int search_child(
    ...
    int raw = same_player
        ? PVS::eval_ctx(child, depth, history, ply, ctx, p, alpha, beta)
        : PVS::eval_ctx(child, depth, history, ply, ctx, p, -beta, -alpha);
...
int PVS::eval_ctx(
    ...
    auto actions = ordered_actions(state);
    ...
    for(auto& action : actions){
        ...
        if(first_child){
            ...
            score = search_child(
                next, depth - 1, history, ply + 1, ctx, p,
                alpha, alpha + 1, same
            );
